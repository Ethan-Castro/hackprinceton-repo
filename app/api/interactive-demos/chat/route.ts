...
export async function POST(req: Request) {
  try {
    const {
      ...
      articleContext,
    }: { messages: UIMessage[]; modelId: string; articleContext?: string } = await req.json();
    ...
    const systemContext = articleContext
      ? `## Context: Article Content
The user is asking questions about the following article. Use this content to inform your answers and generate interactive demos that illustrate the concepts discussed in the article.

${articleContext}

When the user asks for a demo or example, use the 'generateHtmlPreview' tool to create a live, interactive HTML/CSS/JS demonstration of the concept.
`
      : undefined;

    const { agent, supportsReasoning } = createChatAgent(modelId, systemContext);
